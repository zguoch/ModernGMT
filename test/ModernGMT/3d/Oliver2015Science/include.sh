...
function dataset()
{
    ...
    lat0=`echo -39 -12 |awk '{print $1+$2/60}'`
    lon0=`echo -91 -30 |awk '{print $1+$2/60}'`
    echo "Chile" $lon0 $lat0
    lon_min=`echo $lon0 | awk '{print $1-0.4}'`
    lon_max=`echo $lon0 | awk '{print $1+0.4}'`
    lat_min=`echo $lat0 | awk '{print $1-0.4}'`
    lat_max=`echo $lat0 | awk '{print $1+0.4}'`
    len_lon=`echo $lon_min $lon_max | awk '{print ($2-$1)}'`
    len_lat=`echo $lat_min $lat_max | awk '{print ($2-$1)}'`
    scale_z=3  #z轴与横向的尺度比例，用于计算z轴的长度
    bathy=bathy_chile.nc
    bathycpt=bathy.cpt
