...
gmt begin $figname $fmt
    ...
    gmt grdview ${bathy} -JZ -p -C$bathycpt -N${zmin}+g$color_profile -Qi -I$bathy_azm
    # gmt makecpt -C128 -T-5,0 -N > tmp.cpt
    # gmt grdview ${bathy} -JZ -p -Ctmp.cpt -Qi -I$bathy_azm -t$alpha_bathy 
    # add line
    ...
    # profile along lon
    gmt psbasemap -R$range_LonZLat -JX$width_fig_x/$width_fig_z -JZ$width_fig_y -BS -Ba --MAP_FRAME_PEN=1,black@100 --MAP_ANNOT_OFFSET=-0.2  -py$angle_view/$lat_min
    echo $lonc $zc "lat_min"  | gmt pstext -JZ -p -F+f20p,Helvetica-Bold,red=thinner,$lc_segments+jMC+a0
    ...
    echo $lonc $zc "lat_max"  | gmt pstext -JZ -p -F+f20p,Helvetica-Bold,red=thinner,$lc_segments+jMC+a0
    # dxdy=0.01/0.2
    ...
    echo $latc $zc "lon_min"  | gmt pstext -JZ -p -F+f20p,Helvetica-Bold,red=thinner,$lc_segments+jMC+a0
    gmt psbasemap -R$range_LatZLon -JX$width_fig_y/$width_fig_z -JZ$width_fig_x -BS -Ba --MAP_FRAME_PEN=1,black@100 --MAP_ANNOT_OFFSET=-0.2  -px$angle_view/$lon_max
    echo $latc $zc "lon_max"  | gmt pstext -JZ -p -F+f20p,Helvetica-Bold,red=thinner,$lc_segments+jMC+a0
    # dxdy=0.01/0.2
